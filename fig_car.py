"""test_car.m 相当の図（Car 1音源 → FOA）を SpatialScaper(実測SRIR) で作る。
元 dry mono vs 合成 FOA 4ch(W,Y,Z,X) の波形＋スペクトログラム。
音源は SELD-Data-Generator 自動Carクラスの 131855.wav（CAR_CLIP で変更可）。
"""
import os
import random
...
SR = 24000
CAR_CLIP = "../SELD-Data-Generator/database/FSD50K/FSD50K.dev_audio/233472.wav"  # test_car.m と同じ音源(Car passing by, 先頭10s使用)
OUT_TRACK = "output_compare/foa/car_demo"
OUT_LABEL = "output_compare/labels/car_demo"
FIG = Path("output_compare/figs/compare_car_demo.png")
CHN = ["W", "Y", "Z", "X"]
...
def main():
    ...
    ssc.ref_db = -65
    # Car を1つ強制配置（静止、t=1s 開始、クリップ全長 約26s を使う）
    ...
    print(f"Car event: {Path(e.source_file).name}  t={st:.1f}-{st+du:.1f}s  "
    ...
    fig.suptitle(f"Car (SpatialScaper, METU measured SRIR)   "
    ...
    ax[2, 1].text(0.02, 0.5,
                  ...
                  f"cf. test_car.m = ideal-direction FOA without measured SRIR.",
